...
Defaults: 15% max drawdown, 65% min confidence, 3% max position size, 30% max sector concentration, quarter-Kelly, short-term only (≤2h expiry). All configurable in `src/config/settings.py`.
...
max_time_to_expiry_days = 2/24   # Short-term only (≤2h expiry)
...
- **AI Directional** requires confidence ≥ 65%, edge ≥ 8%, category score ≥ 30, short-term expiry (≤2h), and is gated by drawdown / sector caps. On many days, no markets clear all filters.
