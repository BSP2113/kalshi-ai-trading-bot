...
Defaults: 15% max drawdown, 65% min confidence, 3% max position size, 30% max sector concentration, quarter-Kelly, short-term only (≤5h expiry). All configurable in `src/config/settings.py`.
...
max_time_to_expiry_days = 5/24   # Short-term only (≤5h expiry)
...
- **AI Directional** requires confidence ≥ 65%, edge ≥ 8%, category score ≥ 30, short-term expiry (≤5h), and is gated by drawdown / sector caps. On many days, no markets clear all filters.
